...
struct PlayView: View {
    ...
    private var readiness: some View {
        VStack(alignment: .leading, spacing: 16) {
            SectionHeading(title: "Readiness", detail: "One managed installation")
            StatusRow(title: "Runtime", symbol: "shippingbox.fill", state: model.snapshot.runtime)
            StatusRow(title: "Bottle", symbol: "cube.transparent", state: model.snapshot.bottle)
            StatusRow(title: "Steam", symbol: "person.crop.circle", state: model.snapshot.steam)
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            Image(systemName: state.isReady ? "checkmark.circle.fill" : "circle.dashed")
                .foregroundStyle(state.isReady ? SecundaTheme.aurora : SecundaTheme.secondaryText)
        }
        .padding(.vertical, 3)
    }
}
